    ...
    while True:
        ...
        if show_windows:
            ...
            if key == ord('r'):
                ...
                cap.release()
                try:
                    # OPTION 1: capture_mini returns the new mini_id
                    new_mini_id = mt.capture_mini(
                        camera_index=camera_index,
                        background_blur=BG_cam["blur"],
                        background_bgr=BG_cam["bgr"]
                    )
                    if new_mini_id:
                        print(f"✅ Captured new mini_id={new_mini_id}. Requesting Foundry assignment…")
                        fo.request_assignment(str(new_mini_id))
                    else:
                        print("⚠️ capture_mini did not return a mini_id; cannot request Foundry assignment.")
                finally:
                    cap = cv2.VideoCapture(camera_index)
                    if not cap.isOpened():
