...
def render_hsv_overlap_view(profiles: Dict[str, ColorProfile], width: int = 1280) -> np.ndarray:
    """
    Debug view showing the HSV ranges of all saved band profiles.
    Each band gets a row with:
      - A color swatch (median HSV rendered as BGR)
      - A hue spectrum bar with the band's range(s) highlighted
      - S and V range text
    Overlapping hue regions between different bands are tinted red.
    """
    BAR_H      = 36   # height of each hue bar
    SWATCH_W   = 56   # width of color swatch
    LABEL_W    = 110  # width of name label
    ROW_PAD    = 10   # vertical padding between rows
    HEADER_H   = 48   # hue axis header height
    FOOTER_H   = 28
    FONT       = cv2.FONT_HERSHEY_SIMPLEX

    bar_w = width - LABEL_W - SWATCH_W - 32  # usable hue bar width
    ...
    # ── Header: hue spectrum strip ──────────────────────────────────────────
    for x in range(bar_w):
        h = int(x / bar_w * 179)
        bgr = cv2.cvtColor(np.array([[[h, 220, 210]]], dtype=np.uint8), cv2.COLOR_HSV2BGR)[0][0]
        canvas[8:8+28, LABEL_W + SWATCH_W + 16 + x] = bgr.tolist()
    # Tick marks every 30 hue units
    for hval in range(0, 180, 30):
        x = int(hval / 179 * (bar_w - 1)) + LABEL_W + SWATCH_W + 16
        cv2.line(canvas, (x, 6), (x, 38), (200, 200, 200), 1)
        cv2.putText(canvas, str(hval), (x - 8, 44), FONT, 0.38, (180, 180, 180), 1, cv2.LINE_AA)

    # ── Build overlap map: for each hue pixel, which bands cover it? ────────
    # Shape: (180,) list of sets of band names
    hue_owners: List[set] = [set() for _ in range(180)]
    for name, prof in profiles.items():
        for r in prof.ranges:
            lo_h, hi_h = int(r.lower[0]), int(r.upper[0])
            for h in range(max(0, lo_h), min(179, hi_h) + 1):
    ...
    for row_i, name in enumerate(color_order):
        ...
        # Draw base grey bar
        canvas[y0:y1, LABEL_W + SWATCH_W + 16: LABEL_W + SWATCH_W + 16 + bar_w] = (55, 55, 55)

        # Determine median HSV for swatch from first range
        r0 = prof.ranges[0]
        med_h = (int(r0.lower[0]) + int(r0.upper[0])) // 2
        med_s = (int(r0.lower[1]) + int(r0.upper[1])) // 2
        med_v = (int(r0.lower[2]) + int(r0.upper[2])) // 2
        ...
        # Swatch
        sx = LABEL_W + 6
        canvas[y0 + 4: y1 - 4, sx: sx + SWATCH_W - 8] = swatch_bgr
        cv2.rectangle(canvas, (sx, y0 + 4), (sx + SWATCH_W - 9, y1 - 5), (200, 200, 200), 1)

        # Label
        cv2.putText(canvas, name, (8, y0 + BAR_H // 2 + 6), FONT, 0.58, (230, 230, 230), 1, cv2.LINE_AA)

        # Hue range bar(s)
        for r in prof.ranges:
            lo_h = int(r.lower[0])
            hi_h = int(r.upper[0])
            lo_s = int(r.lower[1]);  hi_s = int(r.upper[1])
            lo_v = int(r.lower[2]);  hi_v = int(r.upper[2])

            x_lo = int(lo_h / 179 * (bar_w - 1)) + LABEL_W + SWATCH_W + 16
            x_hi = int(hi_h / 179 * (bar_w - 1)) + LABEL_W + SWATCH_W + 16

            # Fill the range with the actual hue colors
            for x in range(x_lo, min(x_hi + 1, LABEL_W + SWATCH_W + 16 + bar_w)):
                hpx = int((x - LABEL_W - SWATCH_W - 16) / bar_w * 179)
                overlap = len(hue_owners[hpx]) > 1
                s_draw = med_s
                v_draw = med_v
                if overlap:
                    # Red tint for overlapping hue regions
                    bgr = (0, 0, 220)
                else:
                    bgr = cv2.cvtColor(
                        np.array([[[hpx, s_draw, v_draw]]], dtype=np.uint8), cv2.COLOR_HSV2BGR
                    )[0][0].tolist()
                canvas[y0 + 2: y1 - 2, x] = bgr

            # Border around range
            cv2.rectangle(canvas, (x_lo, y0 + 2), (x_hi, y1 - 3), (255, 255, 255), 1)

            # S/V text to the right of bar
            sv_text = f"S:{lo_s}-{hi_s}  V:{lo_v}-{hi_v}"
            tx = LABEL_W + SWATCH_W + 16 + bar_w + 6
            cv2.putText(canvas, sv_text, (tx, y0 + BAR_H // 2 + 5), FONT, 0.38, (180, 180, 180), 1, cv2.LINE_AA)

    # ── Footer ───────────────────────────────────────────────────────────────
    fy = total_h - FOOTER_H + 18
    cv2.putText(canvas, "RED = overlapping hue ranges between bands", (LABEL_W, fy),
                FONT, 0.42, (80, 80, 220), 1, cv2.LINE_AA)
...
def begin_session(
    ...
    # Tkinter camera preview — shown automatically before ArUco lock
    cam_preview: Optional[TkCameraPreview] = TkCameraPreview()
    _ever_locked = False
    ...
    try:
        for bundle in sess.frames():
            ...
            # ── Tkinter camera preview (auto-shown before lock, auto-closes on lock) ──
            if cam_preview is not None and not cam_preview.is_closed:
                cam_preview.update(
                    bundle.cam_bgr,
                    locked=bool(bundle.locked),
                    marker_count=int(bundle.last_marker_count),
            ...
            show_hsv_overlap_view = bool(tog.get("show_hsv_overlap", False))
            if show_windows and show_hsv_overlap_view and profiles:
                if _do_display:
                    hsv_vis = render_hsv_overlap_view(profiles)
                    ensure_window("HSV ranges", 1280, hsv_vis.shape[0])
                    cv2.imshow("HSV ranges", hsv_vis)
                any_cv_window_open = True
                hsv_overlap_window_open = True
            else:
                if hsv_overlap_window_open:
                    try:
                        cv2.waitKey(1)
                        cv2.destroyWindow("HSV ranges")
                    except Exception:
                        pass
                    hsv_overlap_window_open = False
