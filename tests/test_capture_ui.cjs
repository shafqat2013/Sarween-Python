...
const hooks=new Map();window.Hooks={once(name,callback){hooks.set(name,callback)},on(){}};
...
const tokens=names.map((name,index)=>({id:String(index),name,width:1,height:1,sight:{enabled:true,range:30},async update(values){Object.assign(this,values);window.lastTokenMove={name,...values}}}));tokens.get=id=>tokens.find(token=>token.id===id);
const scale=Math.min(innerWidth/2400,innerHeight/1350);
window.canvas={ready:true,scene:{id:'scene',width:2400,height:1350,grid:{size:50,type:1},tokenVision:true,fog:{exploration:true},tokens},dimensions:{sceneX:0,sceneY:0},stage:{worldTransform:{a:scale,b:0,c:0,d:scale,tx:0,ty:0},pivot:{x:0,y:0}},tokens:{releaseAll(){},get(id){return {control(){window.controlled=(window.controlled||new Set());window.controlled.add(id)}}}},pan(){},async animatePan(){}};
...
(async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    const allowed = ['module.js', 'capture_logic.mjs', 'maps/dnd1.jpg', ...[10,11,12,13].map(id=>`viewport_markers/marker_${id}.png`)];
