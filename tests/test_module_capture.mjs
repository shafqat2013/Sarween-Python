...
const hooks = new Map();
const sent = [];
const scene = {id: "new-scene", width: 1200, height: 800, grid: {type: 1, size: 100}, tokens: participants.map(mini => ({id: mini.tokenId, name: mini.tokenName}))};
const context = vm.createContext({
  ...
  game: {settings: {get(_module, key) {return {viewportMarkersEnabled: true, viewportMarkerSize: 120, viewportMarkerInset: 8}[key];}}},
  canvas: {ready: true, scene, dimensions: {}, stage: {worldTransform: {a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0}}},
  WebSocket: {OPEN: 1},
  setTimeout(callback) {callback(); return 1;}, clearTimeout() {},
});
const source = fs.readFileSync(new URL("../module.js", import.meta.url), "utf8");
const module = new vm.SourceTextModule(source + '\n globalThis.moduleTest = {setSocket(value) {ws = value;}};', {context});
await module.link(() => new vm.SyntheticModule(["CAPTURE_MINIS", "generateCaptureTargets"], function () {
