...
# ──────────────────────────────────────────────────────────────────────────────
# Window helpers (RESIZABLE WITHOUT SNAP-BACK)
# ──────────────────────────────────────────────────────────────────────────────
_WINDOW_SIZED = set()

def ensure_window(name: str, w: int = 1280, h: int = 720):
    """
    Create a WINDOW_NORMAL OpenCV window and set its initial size ONCE.
    After that, user resizing will persist (no snap-back).
    """
...
def begin_session(on_mini_moved, camera_index=None,
    ...
    # Start hidden; we show it after lock (as you requested)
    panel.hide()
    ...
    # Pre-lock we show camera. Post-lock we close it and show panel.
    show_cam_view = True

    # Blended is only meaningful self-hosted; calibration already no-ops in foundry mode
    ...
    while True:
        ...
        # ── Control panel: keep UI responsive and handle actions ───────────────
        if not panel.pump():
            # User hit Exit or closed the control panel window
            break
        ...
        if actions.get("recapture_bg"):
            ...
            if BG_cam is None:
                ...
                bg_now = _warp_gray_blur(cam, H_saved, warp_w, warp_h)
                BG_warp_f32 = bg_now.astype(np.float32)

        if actions.get("capture"):
            # Only makes sense if final_mask_cam exists (i.e., after lock / in normal pipeline)
            if "final_mask_cam" in locals() and final_mask_cam is not None:
                ...
                print("ℹ️ Capture not available until after lock.")
        if actions.get("exit"):
            break
        ...
        if (frame_idx % aruco_interval) == 0:
            ...
            panel.set_status(
                ...
                locked=((last_mask_cam is not None) and (last_mask_warp is not None)),
                marker_count=int(last_marker_count),
                missing_ids=list(last_missing_ids),
            )

            # ── Lock acquisition / loss logic (SEPARATE) ──────────────────────
            ...
            # ── First-time lock: close camera window and show control panel ────
            if (last_mask_cam is not None) and (last_mask_warp is not None) and (not control_panel_shown):
                try:
        ...
        # Identify Debug window (controlled by panel)
        ...
        # Keep blended window in sync (calibration.py now no-ops this in Foundry mode)
        if show_windows:
            if show_blended_view:
